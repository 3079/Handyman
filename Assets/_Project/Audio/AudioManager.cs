...
public class AudioManager : MonoBehaviour
{
    ...
    private IEnumerator Fade(string name, float volume1, float volume2, float duration)
    {
        ...
        while(timeElapsed < duration)
        {
            float volume = Mathf.Lerp(volume1, volume2, timeElapsed / duration);
    ...
    private IEnumerator SwitchTheme(string name, float duration)
    {
        ...
        while (timeElapsed < duration)
        {
            ...
            {
                if (m.name != name)
                    m.source.volume = 1f - fadeCurve.Evaluate(timeElapsed / duration);
                else
                    m.source.volume = fadeCurve.Evaluate(timeElapsed / duration);          
            }
            timeElapsed += Time.deltaTime;
            yield return null;
        }
